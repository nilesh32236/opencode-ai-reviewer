...
5. **Timeouts for long-running operations**: Use an explicit timeout for operations with an intentional finite budget. Normal OpenCode Action/CLI runs intentionally omit the application-level timeout; use `withRetryAndTimeout()` from `lib/src/utils/retry.ts` for bounded retries.
