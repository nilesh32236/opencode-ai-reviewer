...
- It can use a smaller / cheaper model — `opencode/muse-spark-1.3-contributor-free` is fine.
...
1. **Verifier model choice:** Default to `reviewModel` (same as the main review) or to a cheaper `verifyResolutionModel`? Recommendation: separate `verifyResolutionModel` config key, defaulting to `opencode/muse-spark-1.3-contributor-free` (cheap, fast).
