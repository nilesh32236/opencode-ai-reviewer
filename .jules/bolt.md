...
**Learning:** Found that `new Set(changedFiles.filter().map())` and `new Map(oldExports.map())` (which created `[s.name, s]` tuple arrays) in `test-gap-detector.ts` created unnecessary intermediate array allocations, causing GC pressure in a hot path.
...
**Refs:** `TestGapDetector.analyze` (changedTestFileSet, sourceFiles), `TestGapDetector.analyze` (oldByName).
