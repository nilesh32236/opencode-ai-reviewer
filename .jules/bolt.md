...
**Learning:** Found that iterating over strings using `for (const codePoint of text)` allocates iterators and strings inside a loop causing high GC pressure. Computing byte lengths over large strings character by character is O(N). It is much faster to allocate a Buffer from the string, jump to the `maxBytes` length, and walk backwards to the start of the UTF-8 character boundary (checking `(buf[end] & 0xc0) === 0x80`).
