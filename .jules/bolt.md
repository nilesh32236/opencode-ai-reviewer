...
**Action:** When two maps share the same key and are read together per cluster, build them in the same pass. Use single-iteration manual nested loops instead of `flatMap` on sets for efficiency.
**Refs:** `lib/src/pattern-detector/engine.ts:85` (file-type metadata map + flatMap elimination).
