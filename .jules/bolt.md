...
## 2026-08-16 - Optimize array deduplication in pattern discovery
**Learning:** Found that `[...new Set(findings.map((f) => f.message).filter(Boolean))]` iterates over the `findings` array multiple times and creates intermediate arrays for mapping and filtering. Additionally, a prior pass was already collecting these same string values into a Map's keys. Deriving the unique array directly from the Map's keys (`[...freqMap.keys()]`) eliminates both the redundant loop and the intermediate array allocations. Note that if constructing deduplicated arrays manually from a Set in the future, ensure the result is correctly converted back to an Array (e.g. `[...mySet]`) since downstream consumers rely on Array methods like `.length` and `.slice()`.
**Action:** Reuse existing Map keys when extracting unique properties instead of performing redundant map/filter passes and intermediate Set allocations.
