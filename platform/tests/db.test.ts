  ...
  async query<T = TaskRow>(sql: string, params: unknown[] = []): Promise<T[]> {
    if (sql.includes('INSERT INTO tasks')) {
      const row = {
        ...
        repo: (params[1] as string | null) ?? null,
        type: params[2] as string,
        priority: params[3] as number,
        pr_number: (params[4] as number | null) ?? null,
        pr_title: (params[5] as string | null) ?? null,
        head_sha: (params[6] as string | null) ?? null,
        base_branch: (params[7] as string | null) ?? null,
        head_branch: (params[8] as string | null) ?? null,
        trigger_source: (params[9] as string | null) ?? 'webhook',
        triggered_by: (params[10] as string | null) ?? null,
        status: 'queued',
        result_data: null,
        error_message: null,
        workspace_path: null,
