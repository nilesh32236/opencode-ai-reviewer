/**
 * Async `git` process runner for the platform workspace manager. Uses
 * `child_process.execFile` with array args (no shell interpolation), so repo
 * names and branches from webhooks can never inject shell commands.
 */
...
export interface ExecGitOptions {
  ...
  maxBuffer?: number;
  /** Kill the git process after this many ms (default: 120_000). */
  timeout?: number;
...
/**
 * Run a `git` command asynchronously via `execFile`.
 * @param args - Arguments passed to the `git` binary.
 * @param options - Execution options.
 * @returns The raw stdout and stderr on success.
 * @throws When git exits non-zero; the error carries `code`, `stdout`, `stderr`.
 */
