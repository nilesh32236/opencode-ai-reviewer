import type { NotificationsConfig, ReviewIssue, ReviewResult, Severity } from '../types/index.js';
...
export interface NotificationContext {
  ...
  repo: string;
  /** Optional absolute PR URL; defaults to a GitHub URL built from repo/number. */
  url?: string;
}
...
/**
 * Build the default PR URL (GitHub) for a repository/PR pair.
 * @param context - Notification context carrying repo and number.
 * @returns A PR URL string.
 */
...
function findingBullet(issue: ReviewIssue): string {
  return `${issue.severity === 'critical' ? '🔴' : issue.severity === 'important' ? '🟠' : '🔵'} ${issue.severity.toUpperCase()}: \`${issue.file}:${issue.line}\` — ${issue.message}`;
}
  ...
  const blocks: SlackBlock[] = [
    {
      ...
      text: {
        type: 'mrkdwn',
        text: `*OpenCode AI Reviewer — <${prUrl}|#${context.number} ${context.title}>*`,
      },
    },
    {
      type: 'section',
...
export function formatTeamsMessage(
  ...
  const prUrl = context.url ?? defaultPrUrl(context);
  const topFindings = getTopFindings(result.issues, 3);
  const verdict = result.verdict.ready ? '✅ Ready to merge' : '⛔ Changes requested';
...
/**
 * Post a JSON payload to a webhook URL with retry and a per-attempt timeout.
 * Failures are non-critical: they are logged as warnings and reported via the
 * boolean return value instead of throwing.
...
export async function postToWebhook(
  ...
  if (!isHttpUrl(url)) {
    log.warn(`Skipping webhook notification: URL is not http(s): ${redactWebhookUrl(url)}`);
    return false;
  }
  ...
  try {
    ...
    // Drain the response body so connections are reusable even when it is empty.
...
/**
 * Validate that a string is a usable http(s) webhook URL.
 * @param url - Candidate URL string.
 * @returns True when the URL parses with an http/https protocol.
 */
...
export async function sendNotification(
  ...
  if (slackUrl) {
    const ok = await postToWebhook(slackUrl, formatSlackMessage(result, context), logger);
    if (ok) logger.info(`Sent Slack notification for PR #${context.number}`);
  }

  if (teamsUrl) {
    const ok = await postToWebhook(teamsUrl, formatTeamsMessage(result, context), logger);
