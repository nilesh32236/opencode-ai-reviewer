...
Uses OpenCode models (`opencode/muse-spark-1.3-contributor-free`) by default; also supports OpenAI, Anthropic, and Gemini.
...
| `review_model`           | `opencode/muse-spark-1.3-contributor-free`    | Model for PR review                            |
...
**Prerequisites:** Node.js >= 24.21.0 (Node 24 LTS "Krypton"; matches the shipped `node24` action runtime), pnpm >= 10.8.
Upgrade with `nvm install 24.21.0 && nvm use 24.21.0`. A below-floor runtime logs a warning (fail-open grace period); set `toolchain.enforceNodeFloor: true` to enforce. Note: `package.json` `engines` is an install-time gate (hard-fails only with `engine-strict` enabled, otherwise advisory), while the runtime floor check stays warn-only during the grace period. Phased plan: warn-only for one release, then harden toward fail-closed.
