...
All shipped workflows are production-ready with timeouts, concurrency guards, and zero-config defaults — they authenticate with a `repo_token` secret (see the secrets note below). See [examples/basic/review.yml](examples/basic/review.yml) and [examples/advanced/ai-suite.yml](examples/advanced/ai-suite.yml) for ready-to-copy templates that compose these reusable workflows.

> **Secrets configuration:** The reusable workflows authenticate with a `repo_token` secret and accept API keys **only via GitHub Secrets** (`secrets: inherit` or an explicit `secrets:` mapping). `secrets: inherit` forwards the caller's secrets **by their literal name** — it does not forward `GITHUB_TOKEN` under the name `repo_token` — so create a `repo_token` secret (a PAT with the permissions listed in each workflow header) in your repository's *Settings → Secrets and variables → Actions*. Configure `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, and `GEMINI_API_KEY` the same way if you use OpenAI, Anthropic, or Gemini models. The default OpenCode model (`opencode/deepseek-v4-flash-free`) requires no external API key.
