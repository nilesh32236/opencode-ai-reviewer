...
| `fail_on_severity`       | `critical`                           | Fail the action (exit code 1) when findings at or above this severity exist: `off`, `critical`, `important`, or `minor`. Use `off` to never fail from findings. See [Branch Protection](#branch-protection-required-status-check). |
...
Set the threshold with the `fail_on_severity` input:
...
- uses: opencode-ai/opencode-ai-reviewer@v1
  with:
    mode: review
    github_token: ${{ secrets.GITHUB_TOKEN }}
    fail_on_severity: critical   # off | critical | important | minor (default: critical)
```

- `critical` (default): fails only when critical findings exist.
- `important`: fails when important or critical findings exist.
- `minor`: fails when any finding exists.
- `off`: never fails from findings (preserves the legacy behavior).

The same knob is available per-repository via `.opencode-reviewer.yml`:
...
For the GitHub App, set the `FAIL_ON_SEVERITY` environment variable (default `critical`).
